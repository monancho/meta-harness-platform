...
function writeRunArtifacts({ runDir, runId, adapter, task, status, reasonCode, message, changedFiles = [], verify = [], patch = '', mode, tmpDir, targetRoot, extraArtifacts = [] }){
...
const codexAdapter = {
  ...
  collectArtifacts(ctx, prepared, executed){
    ...
    return { patch, changedFiles, changedCheck, executed };
  },
  summarize(ctx, prepared, collected){
    return writeRunArtifacts({
      runDir: ctx.runDir,
